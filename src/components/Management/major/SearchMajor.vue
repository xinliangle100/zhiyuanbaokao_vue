    ...
    <el-row :gutter="24">
            ...
            <el-col :span="18">
              <el-row :gutter="24">
                <el-col :span="6" v-for="item3 in item2.children" :key="item3.code" style="margin: 10px;">
                  <a href="#" @click="goMajorChange(item3.code)">{{ item3.name }}</a>
                </el-col>
              </el-row>
            </el-col>
          </el-row>
      ...
      <el-col :span="7">
        ...
        <el-row v-for="item in queryData" :key="item.code"
          ...
          <a href="#" @click="goMajorChange(item.code)">{{ item.name }}</a>
        </el-row>
        <el-col :span="1"></el-col>
      </el-col>
    </el-row>
...
const goMajorChange = (code: string) => {
  window.open(`/MajorChange/${code}`, '_blank'); // -> /user/eduardo
}
// 回到顶部的操作
const scrollToTop = () => {
